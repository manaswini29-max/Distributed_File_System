...
static void *conn_handler(void *arg)
{
    ...
    while (1)
    {
        ...
        //     int midx = find_meta(fname);
        //     if (midx < 0)
        //         midx = create_meta_if_missing(fname, "unknown");
        //     FileMeta *m = &files[midx];
        ...
        else if (strncmp(buf, "WRITE ", 6) == 0)
        {
            ...
            // Collect all input BEFORE acquiring any lock
            char accum[BUFFER_SIZE * 2];
            accum[0] = '\0';
            while (1)
            {
            ...
            // Now get metadata and acquire lock
            int midx = find_meta(fname);
            if (midx < 0)
                midx = create_meta_if_missing(fname, "unknown");
            FileMeta *m = &files[midx];
            ...
            // *** REMOVED THE DUPLICATE RECV LOOP THAT WAS HERE ***
...
int main(void)
{
    ...
    register_with_nm(NM_IP, NM_PORT, "10.106.114.123", SS_PORT);
